...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		for(int i = 0; i <= s.length(); i++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				buffer.insert(i, (char)charCode);
					
				if(!currentList.contains(buffer.toString()) && (!wordsOnly||dict.isWord(buffer.toString()))) {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size() < numSuggestions && count < THRESHOLD) {
			...
			List<String> list = distanceOne(current, false);
			for(String i : list) {
				if(!visited.contains(i)) {
					visited.add(i);
					queue.add(i);
					if(retList.size() < numSuggestions && dict.isWord(i)) {
						retList.add(i);
					}
				}
			}
